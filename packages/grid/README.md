# @easygrid/grid —— React Data Grid 组件库

这是 EasyGrid 的可发布组件库包，提供类似 Airtable 的可编辑数据表格能力。仓库内配有演示应用 `apps/grid-demo` 用于本地体验。
...
```bash
pnpm add @easygrid/grid react react-dom
```

使用示例：

```tsx
import { DataGrid, useDataGrid } from '@easygrid/grid'

type Row = { id: string; name: string }

const columns = [
  { id: 'name', accessorKey: 'name', header: 'Name', minSize: 150 }
]
const data = [{ id: '1', name: 'Alice' }]

export function Example() {
  const { table, ...gridProps } = useDataGrid<Row>({ data, columns })
  return <DataGrid {...gridProps} table={table} height={400} />
}
```
...
```bash
pnpm -w dev --filter @easygrid/grid-demo
```
...
- **React 18+** - 前端框架（peerDependencies）
- **TypeScript** - 类型安全
- **Vite** - 构建工具
- **Tailwind CSS** - 样式框架
- **shadcn/ui** - UI 组件库
...
欢迎提交 Issue 和 Pull Request 改进本组件库。
