...
export const selectCellRenderer: IInternalCellRenderer<ISelectCell> = {
  ...
  measure: (cell: ISelectCell, props: ICellMeasureProps) => {
    ...
    if (!Array.isArray(displayData) || !displayData.length) {return { width, height, totalHeight: height };}
    ...
    const cacheKey = `${String(width)}-${Array.isArray(displayData) ? displayData.join(',') : String(displayData || '')}`;
    const positions: ISelectRegion[] = [];

    for (const text of Array.isArray(displayData) ? displayData : []) {
  ...
  draw: (cell: ISelectCell, props: ICellRenderProps) => {
    ...
    const clipEnable = !isActive && Array.isArray(displayData) && displayData.length;
