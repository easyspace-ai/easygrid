  ...
  return (
    ...
    >
      {visibleCells.map((cell, colIndex) => {
        ...
        // 判断是否为最后一列，如果是则不显示右边框（除非有添加列按钮）
        const isLastCell = colIndex === totalVisibleCells - 1;
        // 如果有添加列按钮，所有数据列都显示右边框；否则只有非最后一列显示
        const shouldShowBorder = hasAddColumn 
          ? true  // 有添加列按钮时，所有数据列都显示右边框
          : !isLastCell;  // 没有添加列按钮时，只有非最后一列显示
        ...
        return (
          <div
            ...
            className={cn({
              ...
              "border-r": cell.column.id !== "select" && shouldShowBorder,
            })}
            style={{
