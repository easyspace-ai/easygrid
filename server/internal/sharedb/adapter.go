...
type Adapter interface {
	...
	// GetOps 获取操作历史
	GetOps(ctx context.Context, collection string, id string, from, to int64) ([]*Operation, error)

	// SkipPoll 跳过轮询优化
	SkipPoll(ctx context.Context, collection string, id string, op *Operation, query interface{}) bool
...
type PubSub interface {
	// Publish 发布消息
	Publish(ctx context.Context, channels []string, op *Operation) error

	// Subscribe 订阅频道
	Subscribe(ctx context.Context, channel string, handler func(*Operation)) error
...
type Service interface {
	...
	// SubmitOp 提交操作
	SubmitOp(ctx context.Context, collection, docID string, op *Operation) error

	// PublishOp 发布操作
	PublishOp(ctx context.Context, collection, docID string, op *Operation) error
