...
func (s *MigrateService) addSupplementaryIndexes(db *gorm.DB) error {
	...
	indexes := []string{
		...
		"CREATE INDEX IF NOT EXISTS idx_record_history_table_record_created ON record_history(table_id, record_id, created_time DESC)",
		"CREATE INDEX IF NOT EXISTS idx_record_history_table_created ON record_history(table_id, created_time DESC)",
